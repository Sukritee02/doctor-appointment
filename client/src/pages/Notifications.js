...
function Notifications() {
  ...
  const  markAllAsSeen = async()=>{
    try {
      ...
      const response = await axios.post('/api/user/mark-all-notofications-as-seen', {usersId : user._id},
        {
          headers: {
  ...
  const  deleteAll = async()=>{
    try {
      dispatch(showLoading());
      const response = await axios.post('/api/user/delete-all-notifications', {usersId : user._id},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          }
